				...
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							//log.info("{} : " + portStats.get(npt).getLinkSpeedBitsPerSec().toString() + " , " +
								//	portStats.get(npt).getBitsPerSecondTx().toString() + " , " + portStats.get(npt).getBitsPerSecondRx().toString() , npt );
