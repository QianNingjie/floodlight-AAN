...
//                        log.info("rawLinkSpeed = " + rawLinkSpeed);
                    }
                }
                for (Link link : links.get(npt)) {
                    if (link == null) {
...
//                    log.info(npt + " : bpsTx = {} bps, cost = {} Mbps",  bpsTx, (int) (bpsTx / Math.pow(10, 6)));
                }
                for (Link link : links.get(npt)) {
                    if (link == null) {
                        continue;
                    ...
                    int cost = (int) (bpsTx / Math.pow(10, 6));
                    linkCost.put(link, cost);

//                    log.info("cost = {}, link : {}", linkCost.get(link), link );
                }
            }
            return linkCost;
        ...
        case UTILIZATION:
            ...
            for (NodePortTuple npt : links.keySet()) {
                ...
                if (spb != null) {
                    bpsTx = spb.getBitsPerSecondTx().getValue();
                    log.info("bpsTx = {} bps, {}MBps", bpsTx, (int) (bpsTx / 10^6) / 8);
                }
                for (Link link : links.get(npt)) {
                    if (link == null) {
                        continue;
    ...
    private List<Path> yens(DatapathId src, DatapathId dst, Integer K, Archipelago aSrc, Archipelago aDst) {
        ...
        for (Path path : A) {
//        	System.out.println("topology " + A.indexOf(path));
